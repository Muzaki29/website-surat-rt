...
export function WargaNotificationBell() {
  ...
  const load = useCallback(async () => {
    const res = await fetch("/api/notifikasi/warga");
    if (!res.ok) return;
  ...
  async function markAllRead() {
    await fetch("/api/notifikasi/warga", {
      method: "PATCH",
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-[min(22rem,calc(100vw-2rem))] rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-xl">
          <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
            <p className="text-sm font-semibold">Notifikasi Anda</p>
            {unread > 0 && (
              <button
                type="button"
                onClick={markAllRead}
              ...
              items.map((n) => (
                <li key={n.id}>
                  <Link
                    ...
                    className={`block px-4 py-3 hover:bg-[var(--color-surface-muted)] ${!n.dibaca ? "bg-sky-50/40" : ""}`}
                  >
                    <p className="text-sm font-medium">{n.judul}</p>
                    <p className="mt-0.5 text-xs text-[var(--color-text-muted)]">{n.pesan}</p>
                  </Link>
                </li>
              ))
            )}
